#!/usr/bin/env bash
# Launch all systems: Fleet (all-in-one), MES, WMS, Traceability, QMS, CMMS, PLM, ERP.
...
[[ ! -x ./bin/plm ]]          && go build -o bin/plm ./cmd/plm
[[ ! -x ./bin/erp ]]          && go build -o bin/erp ./cmd/erp
...
cleanup() {
  ...
  kill $PID_FLEET $PID_MES $PID_WMS $PID_TRACE $PID_QMS $PID_CMMS $PID_PLM $PID_ERP 2>/dev/null || true
  wait $PID_FLEET $PID_MES $PID_WMS $PID_TRACE $PID_QMS $PID_CMMS $PID_PLM $PID_ERP 2>/dev/null || true
  exit 0
}
...
echo "  PLM:               http://localhost:8086  (products, BOM, ECOs)"
echo "  ERP:               http://localhost:8087  (orders, submit to MES)"
echo ""
